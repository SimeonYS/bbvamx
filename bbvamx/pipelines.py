...
class BbvamxPipeline:

    # Database setup
    conn = sqlite3.connect('bbvamx.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bbvamx`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bbvamx WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bbvamx (date, title, link, content)"
